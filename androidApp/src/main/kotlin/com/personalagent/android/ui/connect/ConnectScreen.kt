...
fun ConnectScreen(
    ...
    Column(
        Modifier
            .fillMaxSize()
            // Edge-to-edge: this screen has no Scaffold, so it owns its own insets —
            // keep clear of the status/nav bars, and lift above the keyboard when it
            // opens, applied ONCE as the union (max per side) to avoid double-count.
