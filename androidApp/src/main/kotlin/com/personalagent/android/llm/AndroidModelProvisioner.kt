...
class AndroidModelProvisioner(context: Context) : ModelProvisioner {
    ...
    private fun installedFile(option: ModelOption): File =
        File(
            ...
            "${LlmModelProvisioning.MODEL_SUBDIR}/${option.fileName}",
        )
    ...
    override fun provision(option: ModelOption, wifiOnly: Boolean): Flow<ProvisionState> = flow {
        ...
        val tmp = File(dest.parentFile, "${option.fileName}.part")

        var connection: HttpURLConnection? = null
        try {
            connection = (URL(option.sourceUrl).openConnection() as HttpURLConnection).apply {
            ...
            emit(ProvisionState.Installed(dest.absolutePath))
        } catch (e: Exception) {
            tmp.delete()
