 ...
 * OWNED BY THE SIBLING (`feat/model-provisioning-shared`). This file is a
 * **standalone copy** added on the `feat/model-provisioning-ios` branch ONLY so
 * the iOS onboarding / Settings UI + iosMain bridge compile in isolation. The
 * `feat/model-provisioning-android` branch carries a byte-identical copy for the
 * same reason — so BOTH UI branches and the shared branch will collide here.
 * When the sibling's canonical version lands, **delete this file** and keep
 * theirs — the UI + bridge are written to these exact signatures, so a matching
 * contract is a drop-in. If shapes diverge, the sibling's wins; reconcile the UI
 * and the iosMain adapter to it (see the notes on each type below).
 *
