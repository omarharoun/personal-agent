...
struct SupportResponseView: View {
    ...
    var body: some View {
        NavigationStack {
            ...
            .confirmationDialog(
                ...
                if let phone = contact.phoneNumber, let tel = CrisisContactLauncher.telURL(phone) {
                    Button("Call \(contact.name)") { openURL(tel) }
                }
                if let phone = contact.phoneNumber, let sms = CrisisContactLauncher.smsURL(phone) {
    ...
    private var header: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            Text(response.supportiveMessage)
                .font(.title3)
        ...
        VStack(alignment: .leading, spacing: 12) {
            ...
            if model.trustedContacts.isEmpty {
                ...
                ForEach(model.trustedContacts, id: \.id) { contact in
                    ...
                    .disabled(contact.phoneNumber == nil)
                }
            }
        }
    }
    ...
    private func resourceCard(_ resource: CrisisResource) -> some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            HStack(spacing: 12) {
                if let phone = resource.phoneNumber, let tel = CrisisContactLauncher.telURL(phone) {
                    Button { openURL(tel) } label: {
                        Label("Call", systemImage: "phone.fill")
                    }
                    .buttonStyle(.borderedProminent)
                }
                if let sms = resource.smsNumber,
                   let url = CrisisContactLauncher.smsURL(sms, body: resource.smsBody) {
                ...
                if let urlString = resource.url, let url = URL(string: urlString) {
                    Button { openURL(url) } label: {
                        Label("Visit", systemImage: "safari.fill")
                    }
                    .buttonStyle(.bordered)
                }
            }
