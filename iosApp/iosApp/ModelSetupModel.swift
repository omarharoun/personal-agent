...
final class ModelSetupModel: ObservableObject {
    ...
    init(provisioner: ModelProvisioner, onInstalledChange: @escaping () -> Void = {}) {
        ...
        let catalog = ModelCatalog.shared.options
        self.options = catalog
        // Catalog is non-empty by construction; fall back defensively.
        self.selected = catalog.first ?? ModelCatalog.shared.options[0]
        refreshInstalled()
    }
